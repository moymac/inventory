...
import { getUserPermissions, appendToSheet } from "../Calls";
...
export default class UserSelection extends Component<{}> {
  ...
  buttonClick = async () => {
    ...
    if (usnm.length < 3) {
      ...
      let userPermissions = await getUserPermissions(usnm);
      console.log("usrperm", userPermissions);
      if (userPermissions) {
        if (userPermissions[2] == "1") {
          AsyncStorage.setItem("userName", JSON.stringify(usnm));
          this.props.navigation.navigate("WelcomeScreen");
        } else {
          Alert.alert("Unauthorized access", "User not authorized");
        }
      } else {
